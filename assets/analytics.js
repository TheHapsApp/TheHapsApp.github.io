...
/* Self-exclusion: load any page once with ?ph_optout=1 to stop PostHog from
   ...
   Decided synchronously before init so not even the first pageview leaks. */
var phOptedOut = false;
try {
  var phParam = new URLSearchParams(location.search).get('ph_optout');
